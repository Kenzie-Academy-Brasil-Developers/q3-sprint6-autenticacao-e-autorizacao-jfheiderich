...
def create_user():
    ...
    try:
        ...
        if len(validate_keys) != 0:
            return {"error": {"misssing_keys": validate_keys}}, HTTPStatus.BAD_REQUEST
...
def put_home():
    ...
    session: Session = current_app.db.session
    user_to_update = UserModel.query.filter(UserModel.email == data["email"]).first()
    ...
    for key, value in data.items():
        ...
        if key == "email":
            data[key] = value.lower()
        if not value:
            return {"error": f"{key.upper()} is empty!"}, HTTPStatus.BAD_REQUEST
        ...
        if type(value) != str:
            return {"error": "The values only accept Strings"}, HTTPStatus.BAD_REQUEST
